...
function EditMyTravel() {
  ...
  useEffect(() => {
    ...
    const getTravel = async ()=>{
      ...
      const resData = await axios.get(`https://travel-service-server-by-prisma-murex.vercel.app/travel/one/${travelId}`)
   ...
   //Validate UI แล้วค่อยส่งข้อมูลไปให้ API ทีฝั่ง Back-end
    if( travelPlace.trim().length == 0){
      ...
      try{
        ...
        const response = await axios.put(`https://travel-service-server-by-prisma-murex.vercel.app/travel/${travelId}`, formData, {
          headers: {
